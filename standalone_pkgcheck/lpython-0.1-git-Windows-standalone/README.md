...
- ### Windows
    ...
    - Build a standalone Windows package from the installed tree:

        ```bash
        cpack -G ZIP
        ```

      This creates a self-contained package with `bin/`, `share/`, and
      `install-lpython.ps1`. Extract the archive anywhere and run:

        ```powershell
        powershell -ExecutionPolicy Bypass -File .\install-lpython.ps1
        ```

      By default, the script installs LPython to
      `%LocalAppData%\Programs\LPython` and adds its `bin` directory to the
      user `PATH`. The installed `lpython` command uses a wrapper that can
      initialize the Visual Studio build tools automatically before invoking
      the packaged compiler binary.
      On systems without the Microsoft VC++ runtime already installed, you may
      also need the Microsoft Visual C++ Redistributable.

    - Run the test suite from a normal shell:

        ```bash
        micromamba run -n lpython python run_tests.py --no-color
        ```

      The Windows test harness uses the repository `lpython.cmd` wrapper when
      available so runtime tests inherit the Visual Studio environment before
      invoking `src/bin/lpython.exe`. Runtime Python sources are normalized to
      LF during CMake configure so LPython can parse them reliably on Windows.
      `run_with_dbg` tests are currently skipped on Windows because this tree
      does not have runtime stacktrace support enabled there.
- ### Windows with WSL
